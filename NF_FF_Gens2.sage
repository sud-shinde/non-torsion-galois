...
class EllMahler:
    def __init__(self, E, Pol):
        self.oo = MaxExt(E, Pol)
        self.LL = self.oo.LL
        self.curve = E
        self.poly = Pol
        self.rts = Pol.change_ring(self.LL).roots()
        self.div2 = E.division_polynomial(2)/4
        self.Ys = [sqrt(self.div2(r[0])) for r in self.rts]
        self.new_curve = E.change_ring(self.LL)
        pts = []
        for i, ml in enumerate(self.rts):
            pts.append(self.new_curve.point((self.rts[i][0], self.Ys[i], 1)))
        self.pts = pts
        self.hts = [pp.height() for pp in pts]
        self.htsf = [pp*self.LL.degree() for pp in self.hts]
