...
#lst is the list of possible columns (or rows) in the matrice attached to an automorphism
BG = GL(dn,ZZ)

## FF is a symbolic degree 8 defining polynomial of Q(E[f]) for a quadratic polynomial f.
...
"""
Make an automorphsim sigma act on a point pt on the curve cur.
"""
...
def fact_pat(tp):
"""
factorization pattern of a polynomial tp. Output is a list containing tuples (d, m) 
where d is the degree of a factor of tp and m is the multiplicity of the factor d.
"""
    ll = list(factor(tp))
